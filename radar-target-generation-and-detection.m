...
R_res = 1;
V_max = 100;
T_dist = 110; % distance to the target
T_v = -20; % spped of the target
%speed of light = 3e8
c = 3e8;
...
for i=1:length(t)         
    ...
    fb = (2 * T_dist * B) / (c * Tchirp);
    td(i) = (fb * Tchirp) / B;
    r_t(i) = (c * Tchirp * fb) / (2 * B);
...
M_R = reshape(Mix, [Nr, Nd]);

 % *%TODO* :
%run the FFT on the beat signal along the range bins dimension (Nr) and
%normalize.

signal_fft = fft2(M_R, Nr, 1);
...
signal_fft  = signal_fft(1, 1:length(signal_fft)/2-1);
...
for r=1:rows
    for c=1:cols
        ...
        % geet training cells sum value
        [train_value, t_c] = subarea(RDM, c, r, guac_num + trc_num);
        
        train_value = (train_value - guard_value) / (t_c - g_c - 1);
...
% This point is not necessary due to the sliding window care about this!
